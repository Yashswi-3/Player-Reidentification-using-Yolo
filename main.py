...
class PlayerTracker:
    ...
    def extract_multi_crop_features(self, frame, bbox):
        ...
        crops = [
            [x1, y1, x2, y2],
            [max(0, x1 - int(width*0.1)), max(0, y1 - int(height*0.1)),
             min(frame.shape[1], x2 + int(width*0.1)), min(frame.shape[0], y2 + int(height*0.1))],
            [x1, y1, x2, y1 + int(height*0.7)]
        ]
        features_list = []
        ...
        for i, (bbox, features, conf) in enumerate(unmatched_detections):
            ...
            for old_id, stored_data in self.disappeared_tracks.items():
                ...
                if 'last_bbox' in stored_data:
                    ...
                    spatial_consistency = 1 / (1 + distance / 200)
    ...
    def save_tracking_statistics(self, output_path="output/tracking_stats.json"):
        ...
        stats = {
            ...
            'similarity_logs': self.similarity_logs[-100:]
        }
